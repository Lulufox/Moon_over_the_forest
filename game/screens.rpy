...
screen navigation():
    ...
    vbox:
        ...
        if main_menu:
            ...
            textbutton _("{size=28}Начать{/size}") action Start()

        else:

            textbutton _("{size=28}История{/size}") action ShowMenu("history")

            textbutton _("{size=28}Сохранить{/size}") action ShowMenu("save")

        textbutton _("{size=28}Загрузить{/size}") action ShowMenu("load")

        textbutton _("{size=28}Настройки{/size}") action ShowMenu("preferences")

        if _in_replay:

            textbutton _("{size=28}Завершить повтор{/size}") action EndReplay(confirm=True)

        elif not main_menu:

            textbutton _("{size=28}Главное меню{/size}") action MainMenu()

        textbutton _("{size=28}Об игре{/size}") action ShowMenu("about")
        ...
        if renpy.variant("pc"):
            ...
            textbutton _("{size=28}Выход{/size}") action Quit(confirm=not main_menu)
...
screen about():
    ...
    use game_menu(_("Об игре"), scroll="viewport"):
        ...
        vbox:
            ...
            label "{size=38}[config.name!t]{/size}"
            #text _("Версия [config.version!t]\n")
            text _("{size=15}Демо-версия\n{/size}")
...
style notify_frame:
    ...
    background Frame("gui/notify1.png", gui.notify_frame_borders, tile=gui.frame_tile)
